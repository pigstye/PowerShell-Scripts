<#
.SYNOPSIS
Various functions to help with network tasks
.DESCRIPTION
Various functions to help with network tasks - dot source it to load
	ConvertTo-BinaryIP
	get-IP2Long
	get-Long2IP
	get-Mask2Cidr
...
<#
.SYNOPSIS
Returns list of IP Address in that range
#> 
	If ($IP.Contains("/"))  {    
		$Temp = $IP.Split("/")    
		$IP = $Temp[0]    
...
<#
List number of IP address in a network range
#>
