...
This will take a looooong time if you point it at many files.

#>

param([Parameter(Mandatory=$True,ValueFromPipeline=$True,ValueFromPipelinebyPropertyName=$True)][string]$fileSpec)
process {
	...
	$files = Get-ChildItem $fileSpec -Recurse -Force -ErrorAction SilentlyContinue
